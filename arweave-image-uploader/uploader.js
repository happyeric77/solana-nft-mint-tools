...
const getNftName = (name) => `ColorfulLife DAO #${name}`;

const getMetadata = (name, imageUrl, attributes) => ({
  name: getNftName(name),
  symbol: "CFL",
  description: "ColorfulLife DAO",
  seller_fee_basis_points: 100,
  external_url: "https://colorfullife.ml",
  attributes,
  collection: {
    name: "ColorfulLife",
    family: "DAO",
  ...
  properties: {
    ...
    creators: [
      {
        address: "DJrgFBpTEZQS41GjaeJQTUQGAziGjne8ihh48dANFdRk",
        share: 100,
      },
    ],
  },
...
const iterateOverItems = async () => {
  try {
    ...
    const imageUrl = id ? `https://arweave.net/${id}` : undefined;
    const collectionName = "ColorfulLife NFT DAO";
